...
int main(int argc, char *argv[]) {
  ...
  for (int arg = 1; arg < argc; ++arg) {
    ...
    case '-': { // long option --name text
      ...
      if (*nam != '\0') // Do nothing if name is empty
      {
        ...
        while (arg < argc - 1 && *argv[arg + 1] != '-') {
          ...
          if (!txt.empty())
            txt += ' '; //..space separated if more than one
          txt += argv[++arg];
        }
    ...
    // since we know they take one parameter, and are the most likely options to
    // be misplaced
    case 'f':
    case 'l':
      p = argv[arg] + 2;
...
void help() {
  ...
  if (pDefault)
    cout << pDefault->TargetClassDescription(); // some more options probably
                                                // for OBMol
 ...
 * \par SYNOPSIS
 *
 * \b babel [-H<help-options>] [-V] [-m] [-d] [-h] [-p] [-s<SMARTS-pattern>]
 * [-v<SMARTS-pattern>] [-f<#> -l<#>] [-c] [-x<format-options>] [-i<input-type>]
 * \<infile\> [-o<output-type>] -O\<outfile\>
 *
 * \par DESCRIPTION
 *
