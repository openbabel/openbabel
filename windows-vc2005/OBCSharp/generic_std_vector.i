...
%define SWIG_STD_VECTOR_MINIMUM_INTERNAL_GENERIC(CONST_REFERENCE_TYPE, CSTYPE, CTYPE...)
...
#else
  public $csclassname(System.Collections.Generic.IEnumerable<CSTYPE> c) : this()
#endif
  {
    if (c == null)
...
//ForEach method to simplify working around the foreach/delegate interaction "bug"
...
#endif
  {
    ...
    private CSTYPE currentObject;
    private int currentSize;

    public $csclassnameEnumerator($csclassname collection) {
      collectionRef = collection;
      currentIndex = -1;
      currentObject = default(CSTYPE);
    ...
    public bool MoveNext() {
	  ...
	  bool moveOkay = currentIndex+1 < size;
      if(moveOkay) {
        currentIndex++;
...
SWIG_STD_VECTOR_MINIMUM_INTERNAL_GENERIC(const value_type&, CSTYPE, CTYPE)
%enddef
...
namespace std {
  ...
  template<> class vector<bool> {
    SWIG_STD_VECTOR_MINIMUM_INTERNAL_GENERIC(bool, bool, bool)
