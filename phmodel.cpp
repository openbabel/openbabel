...
OEPhModel::~OEPhModel()
{
  vector<OEChemTsfm*>::iterator k;
  for (k = _vtsfm.begin();k != _vtsfm.end();k++)     delete *k;
    
  vector<pair<OESmartsPattern*,vector<float> > >::iterator m;
  for (m = _vschrg.begin();m != _vschrg.end();m++)   delete m->first;
}

void OEPhModel::ParseLine(char *buffer)
{
  vector<string> vs;
  OESmartsPattern *sp;
  ...
  if (EQn(buffer,"TRANSFORM",7))
    {
      ...
      OEChemTsfm *tsfm = new OEChemTsfm;
	...
	vf.push_back(atof((char*)i->c_str()));
      ...
      _vschrg.push_back(pair<OESmartsPattern*,vector<float> > (sp,vf));
    }
}

void OEPhModel::AssignSeedPartialCharge(OEMol &mol)
{
  if (!_init) Init();
  ...
  vector<pair<OESmartsPattern*,vector<float> > >::iterator i;
...
void OEPhModel::CorrectForPH(OEMol &mol)
{
  if (!_init) Init();
...
bool OEChemTsfm::Apply(OEMol &mol)
{
