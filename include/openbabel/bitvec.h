...
namespace OpenBabel
{
  /** @brief A speed-optimized vector of bits.

      This class implements a fast vector of bits using internally a vector 
      of processor native unsigned words. Any bits which are out of reach of the current
      size are considered to be zero. Streamlined, corrected and documented by 
      kshepherd1@users.sourceforge.net
   */
  class OBERROR OBBitVec
  {
    ...
    public:
       ...
       */
      OBBitVec() : m_set(STARTWORDS, 0)
      { m_size = m_set.size(); }
      /** 
       * Construct a bit vector with a size in bits of @p size_in_bits rounded 
       * up to the nearest word and cleared to all zero bits.
       * @param[in] size_in_bits The number of bits for which to reserve space.
       */
      OBBitVec(unsigned size_in_bits) : m_set(WORDSIZE_OF_BITSIZE(size_in_bits), 0)
      { m_size = m_set.size(); }
      /** 
       * Construct a bit vector which is an exact duplicate of @p bv.
       * @param[in] bv The other bit vector to copy to this.
       */
      OBBitVec(const OBBitVec & bv) : m_size(0)
      { (*this) = bv; }
      /** 
      ...
      /** Searches the vector for the first true value, starting at the @p 
       * bit_offset 'th bit.
       * @param[in] bit_offset The first bit to consider.
       * @return The bit offset of the first true bit at or after @p bit_offset, 
      ...
      {
        if (size_in_words <= m_size)
          return false;
        m_set.resize(size_in_words, 0); // increase the vector with zeroed bits
        m_size = m_set.size();
        return true;
      }
      /** 
	  ...
	  bit_offset &= WORDMASK;
	  rtn = (( m_set[word_offset] >> bit_offset ) & 1);
        }
        return rtn;
      }
      /** 
      ...
      void Clear();
      /** 
       * Inverts the entire vector. Note that this may give unexpected results, 
       * as the vector can be considered to end in an arbitrary number of 
       * zero bits. 
       */
      void Negate()
      {
      ...
      /** 
       * Copy the internal word vector. The copy is appended to @p vec.
       * @param[out] vec a vector of words to which to append the data 
       */
      void GetWords(word_vector & vec)
      {
        vec.insert(vec.end(), m_set.begin(), m_set.end());
      }
  ...
  /** 
   * The Tanimoto coefficient, which may be regarded as the proportion of 
   * the "on-bits" which are shared.
   */
