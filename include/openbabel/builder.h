...
namespace OpenBabel
{
  /** @class OBBuilder builder.cpp <openbabel/builder.h>
  ...
  class OBAPI OBBuilder {
    public:
       ...
       */
      static bool Connect(OBMol &mol, unsigned int a, unsigned int b, Eigen::Vector3d &newpos, int bondOrder = 1);
      /** 
       * Atoms a and b are part of two fragments that are not connected in mol.
       * Connect will translate and rotate the fragment that contains b so that
       * a and b are seperated by a bond. This bond is also added.
       ...
       * for failing: a and b are in the same fragment, they are connected).
       */
      static bool Connect(OBMol &mol, unsigned int a, unsigned int b, int bondOrder = 1);
      /** 
       ...
       * \endcode
       */   
      static bool Swap(OBMol &mol, unsigned int a, unsigned int b, unsigned int c, unsigned int d);
      /** 
      ...
      /** 
       * Currently only corrects double bond chemistry comming from smiles. 
       * (OBBond::IsUp() / OBBond::IsDown()).
       */ 
      static void CorrectStereoBonds(OBMol &mol);
      /** 
       * Currently only corrects atom chirality comming from smiles. 
       * (OBAtom::IsClockwize() / OBBond::IsAntiClockwise())
       */ 
      static void CorrectStereoAtoms(OBMol &mol);
    private:
      //! used to hold the fragments loaded in the constructor
      static std::vector<std::pair<OBSmartsPattern*, std::vector<Eigen::Vector3d> > > m_fragments;
       ...
       * @return The OBBitVec defining the fragment to which a belongs.
       */
      static OBBitVec GetFragment(OBAtom *atom);
      /**
       * Internal function, used by GetFragment().
       */
