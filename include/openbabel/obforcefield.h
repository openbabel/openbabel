...
#include <openbabel/plugin.h>           // full header to derive classes
#include <openbabel/bitvec.h>           // full header for class data as value
#include <openbabel/obfunction.h>       // full header to derive classes
#include <float.h>                      // ??
...
#define IF_OBFF_LOGLVL_HIGH   if(GetLogLevel() >= OBFF_LOGLVL_HIGH)
  ...
  class OBFPRT OBForceField : public OBPlugin, public OBFunction
  {
    ...
    protected:
      ...
      int   m_terms; //!< the terms to calculate 
  ...
  public:
    ...
    /**
     * @brief Overload OBFunction::Eval() with OBForceField::Energy().
     */ 
    ...
    /**
     * @brief Set the force field term to enable.
     */
    void SetTermEnabled(OBForceField::Terms term, bool enable) 
    { 
      if (enable)
        m_terms |= term;
      else if (m_terms & term) // make sure bit is on before xor
          m_terms ^= term;
    }
    /**
     * @brief Enable all force field terms.
     */
    void SetAllTermsEnabled(bool enable)
    {
      if (enable)
        m_terms = BondTerm | AngleTerm | StrBndTerm | TorsionTerm | OOPTerm | VDWTerm | EleTerm;
      else
        m_terms = 0;
    }
     ...
     */
    virtual double Energy(bool gradients = true) = 0;
    /** 
     * @param gradients Set to true when the gradients need to be calculated 
     * 
