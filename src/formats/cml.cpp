...
const char* _RANGLE            = ">";
const char* _SLASH             = "/";
const char* _SPACE_             = " "; //CM was _SPACE VC6 gave syntax error
...
void startElement(string namespaceURI, string localName, string prefix, vector<pair<string,string> > &atts)
{
    ...
    if (currentElem != _EMPTY)
        elementStack.push_back(currentElem);
    parent = currentElem;
...
bool WriteAtom(ostream &ofs, OBAtom* atom, int count)
{
    ...
    if (!outputArray)
    {
        ...
        if (outputCML2)
        {
            ...
            if (charge != 0)
                writeAttribute(ofs, C_FORMALCHARGE, charge);
        ...
        else
        {
            ...
            if (charge != 0)
                writeBuiltin(ofs, C_FORMALCHARGE, charge);
            if (molPtr->HasNonZeroCoords())
            {
                if (strcmp(dimension, C_2D) == 0)
                {
    ...
    if (cmlType == C_CML2 || inputArray)
    {
        for (int i = 0; i < natoms; ++i)
        {
            ...
            if (formalChargeVector.size() > 0)
                atom.SetFormalCharge(formalChargeVector[i]);
            vector3 v;
            if (x2Vector.size() > 0)
                v.SetX(x2Vector[i]);
            if (y2Vector.size() > 0)
    ...
    if (/*cmlType == C_CML2 || */inputArray)
    {
        ...
        for (unsigned int i = 0; i < nbonds; ++i)
        {
            ...
            if (orderVector.size() > 0)
                bondPtr->SetBO(getBabelBondOrder(orderVector[i]));
...
int getLengthIndex(OBAtom* a0, OBAtom* a1)
{
    for (unsigned int i = 0; i < lengthVector.size(); ++i)
    {
        if (a0 == lengthVector[i].first[0] &&
                a1 == lengthVector[i].first[1])
            return i;
        if (a0 == lengthVector[i].first[1] &&
                a1 == lengthVector[i].first[0])
            return i;
    }
    return -1;
}
...
int getAngleIndex(OBAtom* a0, OBAtom* a1, OBAtom* a2)
{
    for (unsigned int i = 0; i < angleVector.size(); ++i)
    {
        if (a0 == angleVector[i].first[0] &&
                ...
                a2 == angleVector[i].first[2])
            return i;
        if (a0 == angleVector[i].first[2] &&
                a1 == angleVector[i].first[1] &&
                a2 == angleVector[i].first[0])
            return i;
    }
    return -1;
}
...
void generateInternals()
{
    ...
    for (internalVector_t::size_type i1 = 0; i1 < internalVector.size(); ++i1)
    {
        // CM i1 was i but VC6 gave redefinition error
        OBInternalCoord* coord = internalVector[i1];
...
bool WriteCML(ostream &ofs,OBMol &mol,const char *dim,const char* xmlOptions)
{
    ...
    if (xmlOptions != 0)
    {
        ...
        if (xo.find("d") < xo.length())
            outputDoctype = true;
        if (xo.find("p") < xo.length())
            outputPretty = true;
        if (xo.find("n") < xo.length())
            outputNamespace = true;
        if (xo.find("c") < xo.length())
            outputPrefix = "cml";
        if (xo.find("a") < xo.length())
            outputArray = true;
        if (xo.find("g") < xo.length())
            outputDebug = true;
        if (xo.find("v") < xo.length())
...
void debug(ostream &ofs)
{
    ...
    for (i2 = 0; i2 < molPtr->NumBonds(); ++i2)
    {
        // CM i2 was i but VC6 gave redefinition error
        OBBond* boPtr = molPtr->GetBond(i2);
        ofs << "<tr>" << endl;
        ofs << "<td>" << i+1 << "</td>" << endl;
