...
namespace OpenBabel
{
    ...
    public:
        ...
        {
            return "http://www.cec.mpg.de/forum/portal.php";
        } // optional
    ...
    bool OrcaOutputFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv)
    {
        ...
        while (ifs.getline(buffer, BUFF_SIZE))
        {
            ...
            if (checkKeywords.find("CARTESIAN COORDINATES (ANGSTROEM)") != notFound)
            {
                ...
                if (unitCell)
                    break; // dont't overwrite unit cell coordinate informations
                if (mol.NumAtoms() == 0)
                {
                    newMol = true;
                }
            ...
            if (checkKeywords.find("ORBITAL ENERGIES") != notFound)
            {
                ...
                if (strstr(buffer, "SPIN UP ORBITALS") != nullptr)
                    m_openShell = true;
            ...
            if (checkKeywords.find("UNIT CELL (ANGSTROM)") != notFound)
            { // file contains unit cell information
                ...
                if (checkNextKeyword.find("CARTESIAN COORDINATES (ANGSTROM)") != notFound)
                {
                    ...
                    tokenize(vs, buffer);
                    while (vs.size() >= 4)
                    { // sometime there are additional infos in the line
                        atom = mol.NewAtom();
                        x = atof((char *)vs[1].c_str());
                        y = atof((char *)vs[2].c_str());
                        z = atof((char *)vs[3].c_str());
