...
Local prototypes.
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
static INCHIGEN_HANDLE INCHI_DECL INCHIGEN_Create(void);

static int INCHI_DECL INCHIGEN_Setup(INCHIGEN_HANDLE HGen, 
                              INCHIGEN_DATA * pGenData, 
                              inchi_Input * pInp);

static int INCHI_DECL INCHIGEN_DoNormalization( INCHIGEN_HANDLE _HGen, INCHIGEN_DATA *pGenData);

static int INCHI_DECL INCHIGEN_DoCanonicalization(INCHIGEN_HANDLE _HGen, INCHIGEN_DATA *pGenData ); 

static int INCHI_DECL INCHIGEN_DoSerialization(INCHIGEN_HANDLE _HGen, 
                                  INCHIGEN_DATA * pGenData, 
                                  inchi_Output * pResults);

static void INCHI_DECL INCHIGEN_Reset(INCHIGEN_HANDLE _HGen, 
                                 INCHIGEN_DATA * pGenData, 
                                 inchi_Output * pResults);

static void INCHI_DECL INCHIGEN_Destroy(INCHIGEN_HANDLE _HGen);
...
int CanonOneStructureINChI(INCHIGEN_CONTROL *HGen, int iINChI, FILE *inp_file);
...
{
#ifdef ONLY_STDINCHI_STDKEY
	return INCHIGEN_Setup(_HGen, pGenData, pInp);
#else
	/* to be filled */
	return _IS_ERROR; 
#endif
...
static int INCHI_DECL INCHIGEN_Setup(INCHIGEN_HANDLE _HGen, INCHIGEN_DATA * pGenData, inchi_Input * pInp) 
...
/*^^^ Set debug output */
   ...
   /* output INChI for the whole structure */
    ...
    if ( nRet != _IS_FATAL && nRet != _IS_ERROR ) {
        nRet1 = CanonOneStructureINChI(HGen, INCHI_BAS, inp_file->f);
        nRet = inchi_max(nRet, nRet1);
    }
...
*/
        ...
        (ip->bTautFlags               & TG_FLAG_RECONNECT_COORD)          ) 
    {
        ...
        nRet1 = CanonOneStructureINChI(HGen, INCHI_REC, inp_file->f);
        nRet = inchi_max(nRet, nRet1);
/*
        if ( nRet != _IS_FATAL && nRet != _IS_ERROR )
                maxINChI = 2;
...
/*^^^ Post-1.02b fix - changed capitalization in the next line */
...
/* implementation */
/* vc6_libstdinchi.def provides export withou cdecl_ prefixes */
...
/* implementation */
/* vc6_libstdinchi.def provides export without PASCAL pasc_ prefixes */
/********************************************************/
INCHIGEN_HANDLE PASCAL pasc_STDINCHIGEN_Create(void) 
{
    return STDINCHIGEN_Create( );
