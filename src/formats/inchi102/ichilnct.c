...
static int INCHI_DECL Get_inchi_Input_FromAuxInfo(char *szInchiAuxInfo, 
												  int bDoNotAddH, 
												  InchiInpData *pInchiInp );

static void INCHI_DECL Free_inchi_Input( inchi_Input *pInp );

/* ^^^*/

/*****************************************************************************************************/
int INCHI_DECL Get_std_inchi_Input_FromAuxInfo
             ( char *szInchiAuxInfo, int bDoNotAddH, InchiInpData *pInchiInp )
{
#ifdef ONLY_STDINCHI_STDKEY
	return Get_inchi_Input_FromAuxInfo(szInchiAuxInfo, bDoNotAddH, pInchiInp );
#else
#endif
}
												  ...
												  InchiInpData *pInchiInp )
{
    ...
    if ( nRet != inchi_Ret_OKAY && nRet != inchi_Ret_WARNING ) {
        Free_std_inchi_Input( pInchiInp->pInp );
        pInchiInp->bChiral = 0;
    }
    bLibInchiSemaphore = 0;
    return nRet;
