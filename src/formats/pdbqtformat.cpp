...
namespace OpenBabel
{
	...
	class PDBQTFormat : public OBMoleculeFormat
	{
		...
		{
			return
			"Autodock Protein Data Bank Q (Partial Charge) & Type\n"
	...
	static bool OuputTree(OBMol& mol, ostream& ofs, map <unsigned int, branch >& tree, unsigned int depth, bool moves_many, bool preserve_original_index);
	static void ContructTree (map <unsigned int, branch >& tree, vector <vector <int> > rigid_fragments, unsigned int root_piece, const OBMol& mol);
	static bool DeleteHydrogens(OBMol & mol);
	static unsigned int FindFragements(OBMol mol, vector <vector <int> >& rigid_fragments);
...
//			het = res->IsHetAtom(atom);
	...
	unsigned int FindFragements(OBMol mol, vector <vector <int> >& rigid_fragments)
	{
	...
	bool OuputTree(OBMol& mol, ostream& ofs, map <unsigned int, branch> & tree, unsigned int depth, bool moves_many, bool preserve_original_index)
	{
	...
	void ContructTree (map <unsigned int, branch>& tree, vector <vector <int> > rigid_fragments, unsigned int root_piece, const OBMol& mol)
	{
...
//		for (unsigned int i = 0; i < depth; i++) {numberAtoms+= tree.at(i).second.first.size();}
	...
	bool PDBQTFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
	{
		...
		unsigned int best_root_atom=FindFragements(mol, rigid_fragments); //the return value is the root atom index
		...
		ContructTree(tree, rigid_fragments, root_piece, mol);

		unsigned int rotatable_bonds=0;
		if (!pConv->IsOption("r",OBConversion::OUTOPTIONS)) {rotatable_bonds=torsdof;}


		if (!OuputTree(mol, ofs, tree, rotatable_bonds, false, pConv->IsOption("p",OBConversion::OUTOPTIONS)) )
		{
			stringstream errorMsg;
