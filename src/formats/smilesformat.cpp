...
public:
	...
	SMIFormat() {OBConversion::RegisterFormat("SMI",this);}
  ...
  if (_prev) //need to add bond 
	{
		...
		*/
		if(arom)
		{
			...
			int val;
			if(prevatom->IsCarbon()) val=4;
			else if(prevatom->IsNitrogen()) val=3;
			else if(prevatom->IsPhosphorus()) val=3;
			else if(prevatom->IsOxygen()) val=2;
			else if(prevatom->IsSulfur()) val=2;
...
public:
	//Register this format type ID
	FIXFormat() {OBConversion::RegisterFormat("FIX",this);}
