...
private:
	struct cdBond
	{
		...
		int	order;
			...
			cdBond() {}
			cdBond(UINT32 bgn, UINT32 e, int ord) : begin(bgn), end (e), order(ord) {}
			~cdBond() {}
	};
...
//						printf("Renumber bond end to real atom\n");						
						...
						pmol->AddBond(atoms[bondsIter->begin], atoms[bondsIter->end], bondsIter->order);
					prevBond = *bondsIter;
				}
				break;
			}
...
//	INT32 x, y;
	...
	pos = new char[size];
	ifs->read(pos, size);
	y = ((pos[3] & 0xff) << 24) | ((long) (pos[2] & 0xff) << 16) | ((long) (pos[1] & 0xff) << 8) | ((long) (pos[0] & 0xff));
	x = ((pos[7] & 0xff) << 24) | ((long) (pos[6] & 0xff) << 16) | ((long) (pos[5] & 0xff) << 8) | ((long) (pos[4] & 0xff));
  x /= 500000;
  y /= 500000;
...
//			printf("End of Object, depth=%d\n", depth);
		...
		else	// Property
		{
			...
			switch(tag)
			{
				...
				case kCDXProp_2DPosition: get2DPosition(ifs, size, x, y); atom.SetVector((double) x, (double) y, (double) 0); break;
...
//			printf("End of Object\n");
		...
		else	// Property
		{
			...
			switch(tag)
			{
				...
				case kCDXProp_Bond_CIPStereochemistry: ifs->seekg(size, ios_base::cur); break;
				...
				case kCDXProp_IgnoreWarnings: ifs->seekg(size, ios_base::cur); break;
				case kCDXProp_MarginWidth: ifs->seekg(size, ios_base::cur); break;
				case kCDXProp_Bond_Display: ifs->seekg(size, ios_base::cur); break;
		...
		if(depth < 1)
		{
			bonds.push_back(cdBond(bgnID,endID,order));
			return 0;
		}
	}
	return -1;
...
/*bool ChemDrawBinaryFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
{
  OBMol* pmol = dynamic_cast<OBMol*>(pOb);
  if(pmol==NULL)
      return false;
