...
public:
	...
	RXNFormat() {OBConversion::RegisterFormat("RXN",this);}
...
bool RXNFormat::ReadMolecule(OBBase* pOb, OBConversion* pConv)
{
	...
	if(pMolFormat==NULL) return false;
...
//	OBConversion MolConv(*pConv); //new copy to use to read associated MOL
  ...
  if(strncmp(buffer,"$RXN",4)) return false; //Has to start with $RXN
  if (!ifs.getline(buffer,BUFF_SIZE)) return(false); //reactiontitle
		pReact->title = buffer;
  ...
  if (!ifs.getline(buffer,BUFF_SIZE)) return(false); //#reactants,products
	if(sscanf(buffer,"%3i%3i",&nReactants,&nProducts) != 2) return false;
	
	if(nReactants + nProducts)
	{
		//Read the first $MOL. The others are read at the end of the previous MOL
		if (!ifs.getline(buffer,BUFF_SIZE)) return false;
		if(strncmp(buffer,"$MOL",4)) return false;
	}
