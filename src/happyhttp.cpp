...
//*****************************************************
//Copyright (c) 2011 Chris Morley
///////////////////////////////////////////////////////
bool IsWebAddress(const string& name, stringstream* ss )
{
  //
  if(name.compare(0, 7, "http://")!=0)
    return false;
  //Find last '.' before '/' or '?' or end
  string::size_type pos, start;
  pos = start = 7;
  while(pos!=string::npos && name[pos]!='/' && name[pos]!='?'&& name[pos]!='#')
    pos = name.find_first_of("./?#", pos+1);
  if(pos!=string::npos)
    HttpToStream(ss, name.substr(start, pos-start), name.substr(pos));
  else
    HttpToStream(ss, name.substr(start), "");
  return true;
}

//////////////////////////////////////////////////////
void OnData(const happyhttp::Response* r, void* pss, const unsigned char* data, int n)
{
  if(r->getstatus()!=200)
    throw happyhttp::Wobbly(r->getreason());
  reinterpret_cast<stringstream*>(pss)->write(reinterpret_cast<const char*>(data), n);
}

///////////////////////////////////////////////////////
void HttpToStream(stringstream* pss, const string& host, const string& datatext)
{
#ifdef WIN32
  WSAData wsaData;
  ...
  try {
    conn.connect();
    conn.setcallbacks(NULL, OnData, NULL, pss);
    conn.request("GET", datatext.c_str(), 0, 0,0);
