/**********************************************************************
binary.cpp - Handle rotamer list and binary base files.
...
bool ReadBinary(unsigned char *buf, OBMol &mol, int size)
{
    ...
    memcpy(&tmp,(unsigned char*)&buf[idx],sizeof(int));
    idx += (int)sizeof(int);
    if (SwabInt)
        tmp = Swab(tmp);
    ...
    int imin[3],imax[3];
    double min[3],max[3];
    memcpy((char*)imin,&buf[idx],sizeof(int)*3);
    idx += (int)sizeof(int)*3;
    memcpy((char*)imax,&buf[idx],sizeof(int)*3);
    idx += (int)sizeof(int)*3;
    ...
    int nconfs,rotmrs;
    memcpy((char*)&nconfs,&buf[idx],sizeof(int));
    idx += (int)sizeof(int);
    memcpy((char*)&rotmrs,&buf[idx],sizeof(int));
    idx += (int)sizeof(int);
    if (SwabInt)
    {
        nconfs = Swab(nconfs);
    ...
    if (nconfs == 1 && !rotmrs) //only a single conformer
    {
        ...
        memcpy((char*)tmpi,&buf[idx],sizeof(int)*natoms);
    ...
    else
    {
        ...
        else
        {
            ...
            memcpy((char*)&nrotors,&buf[idx],sizeof(int));
            idx += (int)sizeof(int);
            if (SwabInt)
                nrotors = Swab(nrotors);
