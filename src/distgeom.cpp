...
distgeom.cpp - Distance Geometry generation and sampling
  ...
  Copyright (C) 2012-2017 by Geoffrey Hutchison
...
#pragma warning(disable : 4244) // warning C4244: '=' : conversion from 'double'
                                // to 'float', possible loss of data
#pragma warning(disable : 4305) // warning C4305: '*=' : truncation from
                                // 'double' to 'float'
...
public:
  ...
  bool debug;
  double maxBoxSize;
  std::vector<OBTetrahedralStereo *> tetra;
  std::vector<OBCisTransStereo *> cistrans;
  OBStereoUnitSet ctSunits, tetSunits;
};

OBDistanceGeometry::OBDistanceGeometry() : _d(NULL) {}

OBDistanceGeometry::OBDistanceGeometry(const OBMol &mol,
...
bool OBDistanceGeometry::Setup(const OBMol &mol, bool useCurrentGeometry) {
  ...
  // Save any known, specified stereochemistry
  // Get TetrahedralStereos and make a vector of corresponding OBStereoUnits
  // Get CisTrans and make a vector of those too
  std::vector<OBGenericData *> vdata =
      _mol.GetAllData(OBGenericDataType::StereoData);
  ...
  for (std::vector<OBGenericData *>::iterator data = vdata.begin();
    ...
    if (((OBStereoBase *)*data)->GetType() == OBStereo::CisTrans) {
      ...
      if (config.specified) {
        _d->cistrans.push_back(ct);
        bond_id = _mol.GetBond(_mol.GetAtomById(config.begin),
                               _mol.GetAtomById(config.end))
                      ->GetId();
        _d->ctSunits.push_back(OBStereoUnit(OBStereo::CisTrans, bond_id));
      }
    }
    ...
    if (((OBStereoBase *)*data)->GetType() == OBStereo::Tetrahedral) {
      ...
      if (config.specified) {
        _d->tetra.push_back(th);
        atom_id = config.center;
        _d->tetSunits.push_back(OBStereoUnit(OBStereo::Tetrahedral, atom_id));
      }
    } // end tetrahedral
...
void OBDistanceGeometry::SetUpperBounds() {
  ...
  // If we're in a unit cell, the maximum distance is 1/2 the longest body
  // diagonal
...
inline double Calculate14DistCis(double ab, double bc, double cd, double B,
                                 double C) {
  double lB = M_PI - B;
  double lC = M_PI - C;
...
// ANGLES are in RADIANS!
inline double Calculate14DistTrans(double ab, double bc, double cd, double B,
                                   double C) {
  double lB = M_PI - B;
  double lC = M_PI - C;
...
void OBDistanceGeometry::Set14Bounds() {
  ...
  for (std::vector<OBCisTransStereo *>::iterator ct = cistrans.begin();
       ct != cistrans.end(); ++ct) {
    OBCisTransStereo::Config config = (*ct)->GetConfig();
    // all of these are specified cis/trans bonds
    // so, get the central bond (bc) and check all the bonded atoms for
    // proper stereo
    b = _mol.GetAtomById(config.begin);
    c = _mol.GetAtomById(config.end);
    FOR_NBORS_OF_ATOM(a, b) {
      ...
      FOR_NBORS_OF_ATOM(d, c) {
        ...
        float lBounds =
            _d->GetLowerBounds(a->GetIdx() - 1, d->GetIdx() - 1) + DIST14_TOL;
        float uBounds =
            _d->GetUpperBounds(a->GetIdx() - 1, d->GetIdx() - 1) - DIST14_TOL;
        if ((*ct)->IsTrans(a, d)) {
          // lower bounds should be trans (current upper bounds)
          _d->SetLowerBounds(a->GetIdx() - 1, d->GetIdx() - 1,
                             uBounds - DIST14_TOL);
        } else if ((*ct)->IsCis(a, d)) {
          // upper bounds should be cis (current lower bounds)
          _d->SetUpperBounds(a->GetIdx() - 1, d->GetIdx() - 1,
                             lBounds + DIST14_TOL);
        }
  ...
  // Now correct ring bonds -- if bc is a ring bond, and a and d are in the same
  // ring, then torsion should be "cis-oid"
  ...
  FOR_RINGS_OF_MOL(r, _mol) {
    ...
    for (a = 0; a < size; ++a) {
      ...
      if (bc->IsAromatic() || bc->GetBondOrder() == 2) {
        ...
        // Adjust the non-ring neighbors too -- these should be out of the ring
        // (i.e., trans-oid) Correct non-ring neighbors too -- these should be
        // out of the ring
        FOR_NBORS_OF_ATOM(nbr, _mol.GetAtom(path[b])) {
          if (nbr->GetIdx() == path[a] || nbr->GetIdx() == path[c])
            continue;
...
// ANGLES are in RADIANS!
inline double Calculate15DistAnyTrans(double ab, double bc, double cd,
                                      double de, double B, double C, double D) {
  // Stolen and adapted from RDKit (http://rdkit.org/)
  // Covered under the BSD license
  double xad = bc - cd * cos(C) - ab * cos(B);
  double yad = cd * sin(C) - ab * sin(B);
...
OBCisTransStereo *OBDistanceGeometry::GetCisTransStereo(OBBond *bond) {
  ...
  for (std::vector<OBCisTransStereo *>::iterator ct = cistrans.begin();
       ct != cistrans.end(); ++ct) {
    OBCisTransStereo::Config config = (*ct)->GetConfig();
    // all of these are specified stereo bonds,
    // so, get the central bond (bc) and check all the bonded atoms for
    // proper stereo
    b = _mol.GetAtomById(config.begin);
    c = _mol.GetAtomById(config.end);
...
void OBDistanceGeometry::Set15Bounds() {
  ...
  FOR_TORSIONS_OF_MOL(t, _mol) {
    ...
    FOR_NBORS_OF_ATOM(e, d) {
      ...
      if (_d->GetLowerBounds((*t)[0], e->GetIdx() - 1) >
          0.01) // we visited this
        continue;
    ...
    FOR_NBORS_OF_ATOM(z, a) {
      if (_mol.GetBond(d, &*z) != NULL)
        continue; // Already handled by 1,2 interaction
      if (_d->GetLowerBounds((*t)[0], z->GetIdx() - 1) >
          0.01) // we visited this
        continue;
  ...
  FOR_ATOMS_OF_MOL(_a, _mol) {
    ...
    FOR_ATOMS_OF_MOL(_b, _mol) {
      ...
      FOR_ATOMS_OF_MOL(_c, _mol) {
        ...
        // Triangle rule: length can't be longer than the sum of the two other
        // legs
        ...
        // Triangle rule: length can't be shorter than the difference between
        // the legs
        if (l_bc < (l_ab - l_ac)) {
          l_bc = l_ab - l_ac;
...
void OBDistanceGeometry::SetLowerBounds() {
  ...
  for (unsigned int i = 0; i < N; ++i) {
    ...
    for (unsigned int j = i + 1; j < N; ++j) {
      ...
      if (!_mol.GetBond(a, b) &&
          _d->GetLowerBounds(i, j) <
              0.4f) { // only check for nonobonded contacts
...
void OBDistanceGeometry::CorrectStereoConstraints(double lambda) {
  // First, grab the saved stereo information (cis-trans and tetrahedral)
  std::vector<OBCisTransStereo *> cistrans = _d->cistrans;
  OBStereoUnitSet ctSunits = _d->ctSunits;

  // Now check for the current stereochemistry based on 3D coordinates
...
void OBDistanceGeometry::AddConformer() {
  ...
  Generate(); // for this new conformer
}

void OBDistanceGeometry::Generate() {
  // create a geometry for this coordinate set
  ...
  while (!finished) {
    ...
    float cosAlpha =
        (SQUARE(lenB) + SQUARE(lenC) - SQUARE(lenA)) / (2.0 * lenB * lenC);
    ...
    if (_d->debug) {
      cerr << " origin " << origin << " second " << second << " third " << third
           << endl;
    ...
    OBAtom *a, *b;
    float lambda;
    const unsigned int maxCount = 250;
    ...
    for (unsigned int count = 0; count < maxCount; ++count) {
      ...
      if (generator.NextFloat() > 0.7) {
        ...
        if (!CheckStereoConstraints()) {
          ...
          converged = false;
          continue; // next loop, maybe adjust coordinates
        }
      } else {
