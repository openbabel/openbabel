...
The fragments are obtained by means of a recursive algorithm. The algorithm
finds all linear fragments up to a size of seven atoms. Cyclic fragments are
also identified by checking if there are ring closures in the linear fragment.
However, the algorithm does not identify branched fragments if there are not
part of a ring. 
...
Duplicate fragments, i.e. fragment generated by the same atoms, are removed
from the fragment list. The only fragment that are kept are the ones which
produce the lowest hash numbers.
The hash number for a fragment is an integer number which is generated from
the atoms and bonds of the fragment. The value of this number depends on the
position, the atomic number and type (aromatic or not) of the atoms of the
fragment, the position and the type of the bonds of the fragments, the
position and the type of the ring closures.
 
The fingerprint is a bitstring of 1021 bits. It is evident that there are
many more possible fragments than 1021, and consequently the hash number is
often higher than 1021. It is easy to obtain a number lower than 1021 from
the hash number, just by dividing the hash number by 1021 and by keeping the
reminder. this reminder is then used to set a bit to one in the fingerprint.
Actually, the size of the bitstring is of 1024 bits but we divide by 1021
because 1021 is a prime number which produces a better hashing than an even
number such as 1024. Consequently, the three last bits of the fingerprint
...
{
    unsigned int anumber=0, bnumber=0, cnumber=0;
    list<AtomRing>::iterator i;
