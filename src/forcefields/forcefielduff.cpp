...
namespace OpenBabel
{
  ...
  double OBForceFieldUFF::E_Bond()
  {
    ...
    for (i = _bondcalculations.begin(); i != _bondcalculations.end(); ++i) {
      ...
      IF_OBFF_LOGLVL_HIGH {
        sprintf(_logbuf, "%-5s %-5s  %4.2f%8.3f   %8.3f     %8.3f   %8.3f   %8.3f\n",
  ...
  double OBForceFieldUFF::E_Torsion() 
  {
    ...
    for (i = _torsioncalculations.begin(); i != _torsioncalculations.end(); ++i) {
      ...
      IF_OBFF_LOGLVL_HIGH {
        sprintf(_logbuf, "%-5s %-5s %-5s %-5s%6.3f       %8.3f     %8.3f\n",
                (*i).a->GetType(), (*i).b->GetType(), 
  ...
  */
  ...
  double OBForceFieldUFF::E_OOP() 
  {
     ...
     IF_OBFF_LOGLVL_HIGH {
       sprintf(_logbuf, "     TOTAL OUT-OF-PLANE BENDING ENERGY = %8.3f\n", energy);
       OBFFLog(_logbuf);
     }
     return energy;
  }
  ...
  double OBForceFieldUFF::E_VDW()
  {
    ...
    for (i = _vdwcalculations.begin(); i != _vdwcalculations.end(); ++i, ++j) {
      ...
      IF_OBFF_LOGLVL_HIGH {
        sprintf(_logbuf, "%-5s %-5s %8.3f  %8.3f  %8.3f\n", (*i).a->GetType(), (*i).b->GetType(), 
